...
df_current <- npn_download_individual_phenometrics(
  ...
  additional_fields = c("Observed_Status_Conflict_Flag", "species_functional_type", "partner_group")
)
...
df_all1 <- df_all %>%
  ...
  group_by(site_id, phenophase_id, species_id) %>% 
...
df_all1 <- inner_join(df_all1, quantiles, by=c('site_id'='site_id', 'phenophase_id'='phenophase_id', 'species_id' = 'species_id'))

#add  outlier23 and early23 binary fields for use in data viz
...
leaf_inds <- subset(df_all1,early_2023==1 & pheno_class_id == 1, select=unique("site_id"))
bloom_inds <- subset(df_all1,early_2023==1 & pheno_class_id == 7, select=unique("site_id"))

#write csv with data filtered to records where there is a 2023 outlier or early record
write_csv(subset(df_all1, site_id %in% leaf_state_spp$site_id | site_id %in% bloom_state_spp$site_id), 
          '2023_vs_2009-2022_flowering_leafing_early&outlier_individual.csv')

#box plot with points for current year - leaf
p = ggplot() +
  geom_boxplot(data = subset(df_all1, site_id %in% leaf_inds$site_id & pheno_class_id == 1 & first_yes_year != 2023), aes(x=site_id, y=first_yes_doy)) +
  geom_point(data = subset(df_all1, early_2023 == 1 & pheno_class_id == 1 ),  #earlier than 95% of records
             aes(x=site_id, y=first_yes_doy, size = 1), color="pink") +
  geom_point(data = subset(df_all1, outlier_2023 == 1 & pheno_class_id == 1 ),  #Tukey outliers
             aes(x=site_id, y=first_yes_doy, size = 1), color="red") +
...
p = ggplot() +
  geom_boxplot(data = subset(df_all1, site_id %in% bloom_inds$site_id & pheno_class_id == 7 & first_yes_year != 2023), 
              aes(x=site_id, y=first_yes_doy)) +
